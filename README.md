...
---
## Build History
* **3/31 v1.9.0**: 보스 몬스터 등장 시 등장 알림 텍스트 추가, 원거리 공격과 돌진 시 사운드 추가, 돌진 중 사망 시 즉각 멈추고 사망모션을 취하도록 수정, GameScene3에서 중복으로 등장하는 보스 몬스터 문제 해결.
* **v1.8.0**: 보스 몬스터의 공격 패턴 추가, (플레이어와 떨어져 있으면 원거리 공격, 6초마다 플레이어 방향으로 돌진), 관통(강화 화살, 검 에너지)에 맞은 몬스터가 중복으로 데미지를 받던 문제 해결.
