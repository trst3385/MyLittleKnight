...
본 프로젝트는 유지보수와 확장을 고려하여 **기능 단위(Feature-based)**로 폴더를 구성하였으며, Assets/_Project 경로 내에서 관리됩니다.

*빌드가 완료된 각 버전들은 BuildVersions 폴더, 혹은 releases 에서 확인이 가능합니다.*

경로: MyLittleKnight → Assets → (GameScene, MainScene) 핵심 스크립트 폴더

<br/>

핵심 스크립트 (Scripts 폴더 내부)

*Assets/_Project/Scripts/ 내에 **단일 책임 원칙(SRP)**에 따라 분류되어 있습니다.*

* Player: 이동, 공격 로직(검/활), 상태 관리 및 체력 시스템

* Enemy: FSM 기반 AI, 스폰 시스템, 몬스터의 스탯(SO)

* Item: 아이템 드랍 및 아이템 상자 상호작용 로직

* Managers: 게임 루프 및 전역 상태(Sound, UI, Game), 몬스터와 장애물의 난이도 동적 강화 로직, 아이템 등장 등 게임의 시스템을 관장하는 싱글톤 매니저들

* MainScene: 메인 화면의 배경용 Player와 몬스터, 배경화면의 페럴렉스<br/>
