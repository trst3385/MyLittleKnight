...
public class SwordWeapon : MonoBehaviour
{
    ...
    public float SwordEnergySpeed = 15f;//검기 에너지 속도
    public Transform SwordEnergySpawnPoint;//검기 에너지 생성 위치 (인스펙터에 연결)
    public GameObject SwordEnergy;//검기 에너지 프리팹 (인스펙터에서 연결)
    ...
    {                        //SwordPoint의 BoxCollider2D를 사용하여 OverlapBox로 충돌 감지
        ...
        lastSwordSkillTime = Time.time;//검공격 후 다시 쿨타임
    }

    private void LaunchSwordEnergy()//검기 에너지를 생성하고 초기 속도 및 방향을 설정하는 함수
    {
        if (SwordEnergy == null)//검기 에너지 프리팹에 연결되있지 않으면 실행불가야!
        {
        ...
        //검기 에너지 생성, swordPowerSpawnPoint 오브젝트가 에너지 생성 위치야
        ...
        if (SpriteRenderer.flipX)//케릭터가 보는 방향에 따라 에너지의 방향 설정
