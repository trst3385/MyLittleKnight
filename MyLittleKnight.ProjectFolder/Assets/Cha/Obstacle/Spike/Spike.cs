...
public class Spike : MonoBehaviour
{
    ...
    [Header("가시 데미지")]
    [SerializeField] private float spikedamage = 1f;//밟았을때의 데미지
    [SerializeField] private float debuffDamage = 0.5f;//가시에 벗어난 후 디버프 데미지
    ...
    IEnumerator ApplySpikeDamage()//밟고 있는 동안 지속적으로 데미지를 주는 코루틴
    {
        ...
        { 
            ...
            playerShield.TakeShieldDamage(spikedamage);//쉴드에 먼저 데미지 적용, 방어력이 0이면 체력으로 데미지 이전
    ...
    IEnumerator ApplyDebuffDamage()//가시함정에서 벗어난 후 남은 시간 동안 데미지를 주는 코루틴
    {
        ...
        while (timer > 0)//while (timer > 0): 타이머가 0보다 클 때까지만 반복(제한된 횟수만큼 데미지를 주기 위함)
        {
            ...
            playerShield.TakeShieldDamage(debuffDamage);//playerShield 스크립트 내부에 방어력이 0이되면 체력으로 보내는 로직이 있어,
