...
public class Item: MonoBehaviour
{
    ...
    private bool isUsed = false;//플레이어의 콜라이더가 두개 있으니 중복으로 아이템 획득을 방지하기 위해 bool 변수사용
                                //콜라이더가 두개니 OnTriggerEnter2D 함수가 두번 발동되는거야
    void Start()
    {
    ...
    {   //이 함수는 이 아이템의 Collider2D (Is Trigger가 체크된)가 다른 Collider2D와 닿았을 때 호출.
        ...
        if (isUsed) return;//함수가 시작하자마자 가장 먼저 실행, 이미 사용 중(isUsed가 true)이면  함수 종료(중복 실행 방지)
                           //if (변수 == true) 대신 if (변수) 라고 축약
                           //

        //닿은 오브젝트가 "Player" 태그를 가지고 있는지 확인
        if (!other.CompareTag("Player")) return;
        
        isUsed = true;//아이템을 획득하면 true가 되서 두번째 콜라이더가 닿더라도,
                      //if (isUsed) return; 때문에 더 이상 OnTriggerEnter2D 함수가 실행되지 않아.
