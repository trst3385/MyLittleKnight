...
public class ObstacleDifficultyManager : MonoBehaviour
{
    ...
    public float SpeedIncreaseInterval = 20f;//속도 증가 간격. 몇 초마다 발사체의 속도를 한 단계씩 올릴지 정하는 시간 간격.
    [Header("발사체 생성 주기 조절")]
    public float InitialSpawnInterval = 3f;//초기 생성 주기. 게임이 시작될 때 발사체가 생성되는 기본 시간 간격.
    public float MinSpawnInterval = 0.5f;//최소 생성 주기. 난이도가 계속 올라가도 이 시간보다 더 짧아지지는 않아.
    public float IntervalDecreaseRate = 0.1f;//주기 감소량. 발사체 생성 주기가 한 번 줄어들 때마다 몇 초씩 줄일지 정하는 값이야.
    ...
    public int DamageIncrease = 1;//데미지 증가량. 발사체의 데미지가 한 번 강해질 때마다 몇씩 증가할지 정하는 값
